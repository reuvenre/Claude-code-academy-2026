...
6. הוסף סקשן "שאלות נפוצות" (3-5 שו"ת בפורמט שאלה→תשובה).
7. הוסף `<CanDo>` — משפט "עכשיו אתה יכול…" (מ‑`frontmatter.canDo`).
8. אם זה השיעור האחרון ברמה — הוסף מבחן קצר (`frontmatter.quiz`, 5-8 שאלות).
