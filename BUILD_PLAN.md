...
**✅ הושלם — 9 שיעורים, quiz של 7 שאלות בשיעור 9.** תהליך העבודה והערות להמשך:
- **אימות:** מורידים את המקור כ-Markdown מלא (`curl https://code.claude.com/docs/en/<page>.md`) וכותבים רק מה שמופיע בו.
  עובדה מדף משני מקבלת קישור inline לדף הרשמי.
- **בדיקה:** `npm run build`, מעלים שרת (`npx next start -p 3100`), ואז `npm run audit -- <level>/<slug> ...`.
  הסקריפט מממש את `/seo-audit` ו-`/rtl-check` על ה-HTML המרונדר. היעד הוא 0 ממצאי 🔴 וגם 0 ממצאי 🟡.
- **סטיות מ-CONTENT_OUTLINE:** המקור של שיעור 3 הוא `terminal-guide` ולא `terminal-config` (שעוסק בהתאמת הטרמינל).
- **"הצעד הבא":** מקשרים רק לשיעורים שקיימים. כשהשיעור הבא עוד לא נכתב, מפנים לעמוד השער ומעדכנים אחר כך.
